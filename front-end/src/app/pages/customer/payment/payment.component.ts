...
export class PaymentComponent {
  ...
  ngOnInit() {
    ...
    if (this.id !== null) {
      // Verifique se id não é null
      this.budgetService.getBudgetByMaintenanceId(this.id).subscribe({
        next: (data) => {
          this.budget = {
            ...data,
            dataRecuperacao:
              this.datePipe.transform(data.dataRecuperacao, 'dd/MM/yyyy') ||
              undefined,
            dataOrcamento:
              this.datePipe.transform(data.dataOrcamento, 'dd/MM/yyyy') ||
              undefined,
            precoOrcado:
              this.currencyPipe.transform(data.precoOrcado, 'BRL', 'symbol') ||
              undefined,
          };
        },
        error: (error) => {
  ...
  alternarInformacoes() {
    ...
    this.valorAlternado = this.mostrarInformacoesAtuais
      ? 'Informações do Conserto'
